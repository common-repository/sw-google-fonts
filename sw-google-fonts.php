...
Plugin URI: 
Version: v3.1.4
Description: Google Fonts plugin allows you to easily add fonts from the Google Font Directory to your Wordpress theme. 
Author: Salvador
...
if (!class_exists('swgooglefonts')) {
    class swgooglefonts {
        //This is where the class variables go, don't forget to use @var to tell what they're for
        /**
        * @var string The options string name for this plugin
        */
        var $optionsName = 'swgooglefonts_options';
        
        /**
        * @var string $localizationDomain Domain used for localization
        */
        var $localizationDomain = "swgooglefonts";
        ...
        */
		...
		var $gf_data_option_name = "swgooglefonts_data";
		
		var $gf_fonts_file = 'webfonts.php';
		
		var $gf_notices = array();
		
		var $gf_filename = 'sw-google-fonts.php';
		...
		var $gf_element_names = array(
			'swgooglefonts_font1' => 'googlefont1',
			'swgooglefonts_font2' => 'googlefont2',
			'swgooglefonts_font3' => 'googlefont3',
			'swgooglefonts_font4' => 'googlefont4',
			'swgooglefonts_font5' => 'googlefont5',
			'swgooglefonts_font6' => 'googlefont6'
		);
        ...
        */
        function swgooglefonts(){$this->__construct();}
        ...
        */        
        function __construct(){
			...
			add_action('wp_enqueue_scripts',array(&$this, 'swgooglefontsstart'));
			add_action("wp_head", array(&$this,"addswgooglefontscss")); 
		...
		function gf_get_fonts_select($name="googlefont"){			
			...
			if($this->gf_fonts && is_array($this->gf_fonts)){
				...
				$out.='<div style="clear:both;"><input class="button-primary" type="submit" name="swgooglefonts_save" value="' . __('Save All Fonts', $this->localizationDomain) . '" /></div>';
		...
		function swgooglefontsstart(){
			$have_selections = false;
			$options = get_option('swgooglefonts_options');
        ...
        */
        function admin_menu_link() {
        	...
        	add_menu_page( 'SW Google Fonts', 'SW Google Fonts Setting', 'manage_options', 'sw-google-fonts', array(&$this,'admin_options_page') , 'dashicons-editor-textcolor');
			...
			if (in_array(substr(basename($_SERVER['REQUEST_URI']), 0, 11), array('plugins.php', 'index.php')) && get_option('wp_google_fonts_global_notification') == 1) {
				?>
					...
					<div class="updated" id="wp_google_fonts_global_notification" style="border:3px solid #317A96;position:relative;background:##3c9cc2;background-color:#3c9cc2;color:#ffffff;height:70px;">
						<a class="notice-dismiss" href="<?php echo admin_url('admin.php?page=add-google-fonts&wp_google_fonts_global_notification=0'); ?>" style="right:165px;top:0;"></a>
						<a href="<?php echo admin_url('admin.php?page=add-google-fonts&wp_google_fonts_global_notification=0'); ?>" style="position:absolute;top:9px;right:15px;color:#ffffff;">Dismiss and go to settings</a>
        ...
        */
        function filter_plugin_actions($links, $file) {
           ...
           $settings_link = '<a href="admin.php?page=add-google-fonts">' . __('Settings') . '</a>';
		...
		function gf_handle_submission($data){
			if (! wp_verify_nonce($_POST['_wpnonce'], 'swgooglefonts-update-options') ) die(__('Whoops! There was a problem with the data you posted. Please go back and try again.', $this->localizationDomain)); 
        ...
        */
        function admin_options_page() { 
			$message = null;
            if(isset($_POST['swgooglefonts_save']) && $_POST['swgooglefonts_save']){
				...
				<table width="650" border="0" cellspacing="0" cellpadding="0">
				  <tr>
					<td>
						...
						<h1><?php _e('SW Google Font Control Panel', 'swgooglefonts'); ?></h1>
						
						
						<form method="post" id="swgooglefonts_options">
						<?php wp_nonce_field('swgooglefonts-update-options'); ?>
...
if (class_exists('swgooglefonts')) {
    $swgooglefonts_var = new swgooglefonts();
}
?>